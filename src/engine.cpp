...
namespace hegel::internal {
    ...
    StateMachineHandle::StateMachineHandle(
        ...
        // A sequential machine: every rule in one concurrency group and the
        // concurrency level fixed at 1. out_concurrency is then always 1, so
        // the drawn level is discarded.
    ...
    int64_t StateMachineHandle::next_rule(const TestCase& tc) {
        ...
        // A sequential machine drives a single worker, index 0.
        scope.raise_for_rc(
