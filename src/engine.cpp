...
namespace hegel::internal {
    ...
    StateMachineHandle::StateMachineHandle(
        ...
        // the concurrency level is fixed at 1 (min == max, so no entropy is
        // drawn for it). The drawn level, always 1, is discarded.
    ...
    int64_t StateMachineHandle::next_rule(const TestCase& tc) {
        ...
        // Worker 0: the sole worker of a sequential machine.
        scope.raise_for_rc(
