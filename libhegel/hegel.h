...
/*
 ...
 failure (the property failed again) or is stale (it passed). Replay several
 blobs by calling this once per blob. A blob whose choices no longer match the
...
hegel_result_t hegel_collection_more(hegel_context_t* ctx,
                                     hegel_test_case_t* tc,
                                     int64_t collection_id, bool* out_more);
...
hegel_result_t hegel_collection_reject(hegel_context_t* ctx,
                                       hegel_test_case_t* tc,
                                       int64_t collection_id, const char* why);
...
hegel_result_t hegel_pool_add(hegel_context_t* ctx, hegel_test_case_t* tc,
                              int64_t pool_id, int64_t* out_variable_id);
...
hegel_new_state_machine(hegel_context_t* ctx, hegel_test_case_t* tc,
                        const char* const* rule_names, size_t num_rules,
                        const char* const* invariant_names,
                        size_t num_invariants, int64_t* out_state_machine_id);
