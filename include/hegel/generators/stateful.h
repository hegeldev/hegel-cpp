...
namespace hegel::stateful {
    ...
    void run(M& machine, TestCase& tc, const RunParams& params = {}) {
        ...
        while (true) {
            // One span per round: the engine deletes or reorders a whole
            // round as a unit. The round's stop decision is drawn inside it.
            ...
            try {
                ...
                // Sequential machine: one worker pulls rules until this
                // round's budget is used up.
                while (true) {
                    ...
                    try {
                        // nest the step's draws under its "Step N" header.
                        {
                        ...
                        print_state(tc, machine, params);
                        check_invariants(
                            tc, "after step " + std::to_string(steps_run),
                            machine, invariants);
                    } catch (const internal::HegelReject&) {
                        tc.note("Rule stopped early due to violated "
                                "assumption.");
