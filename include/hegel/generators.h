...
namespace hegel::generators {
    ...
    *
    * IGenerator's core method is generate(), which produces a single value of type T.
    * 
    * IGenerator also has a schema() method that returns the schema (if there is one).
    * 
    * It's rare that you would need to implement IGenerator yourself; you would typically
    * use pre-existing primitives from the library or compose them using compositors from
    * the library. Even the library only uses IGenerator sparingly; most operations are
    * exposed in Generator, a class that implements IGenerator.
    *
    ...
    * @see Generator Sub-class that implements IGenerator; the thing you typically interact with
    */
    template <typename T>
    struct IGenerator {
        IGenerator() {}
        ...
        * used for generating; this functionality may be used for composing generators,
        * for example.
        * 
    ...
    * an IGenerator (which produces values) and provides combinators  (map(), flatmap(), filter())
    * for transforming and composing generators.
    *
    * @code{.cpp}
    ...
    */
    ...
    public:
        ...
        *
        * Given a Generator&lt;T&gt; and a function from T -> S, creates a Generator&lt;S&gt;.
        * 
        * This works by generating values from the Generator&lt;T&gt; and applying a 
        * transformation to each value.
        * 
        * This is used when you have a function to convert *values* between types. Compare with flatmap().
        * 
        * Here's an example of how you'd use this with built-in strategies:
        * @code{.cpp}
        * Generator<double> halved =                           // Result type Generator<double>
        *     integers<int>()                                  // Input type Generator<int>
        *         .map(
        *             [](int x) { return double(x) / 2.0; }    // transformation: double f(int x)
        *         );
        * @endcode
        *
        ...
        *
        * Given a Generator&lt;T&gt; and a function from T -> Generator&lt;S&gt;, creates a Generator&lt;S&gt;.
        * Useful when generation parameters depend on previously generated values.
        *
        * This is used when you have a function that creates a new Generator based on the value.
        * Compare this with map().
        * 
        *
        * @code{.cpp}
        * Generator<std::string> sized_string =                     // Result type Generator<std::string>
        *     integers<size_t>({.min_value = 1, .max_value = 10})   // Input type Generator<size_t>
        *     .flatmap(
        *         [](size_t len) {                                  // transformation Generator<std::string> f(size_t len)
        *             return text({                                 // text() return type is Generator<std::string>
        *                 .min_size = len,                          // Constructor parameters to text() depend on the value *len*
        *                 .max_size = len
        *             });
        *     });
        * @endcode
        ...
        *
        * Creates a Generator that only produces values satisfying the predicate. The new 
        * Generator has the same type as this Generator.
        *
        * For performance reasons, if 3 consecutive values fail the predicate, Hegel rejects the test case.
        * 
        * So for example, if you want sorted lists of length N, you should generate sorted lists
        * of length N, not generate random lists and filter by a predicate of 'length == N && is_sorted'. 
        * (Although the latter is logically correct, it would be a performance nightmare, 
        * so Hegel doesn't let you do it that way.)
        *
        * @code{.cpp}
        * Generator<int> even =                                  // Return type is same as input type
        *     integers<int>({.min_value = 0, .max_value = 100})  // Input type = Generator<int>
        *     .filter(
        *         [](int x) { return x % 2 == 0; }               // bool predicate(int x)
        *     );
        * @endcode
        *
        * @param pred Predicate that values must satisfy. Signature: bool pred(T value)
    ...
    */
    ...
    public:
        ...
        /// @brief Create from a function
        /// @param fn function that will be called repeatedly to generate values
        /// @param schema schema for this generator; not used in generate(), but may be used when composing this generator
    ...
    *
    * You shouldn't create this directly, but rather use the from_schema() or default_generator() functions.
    *
    * @tparam T The type to generate values for (must be reflect-cpp compatible)
