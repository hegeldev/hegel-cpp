...
namespace hegel::internal {
    ...
    class StateMachineHandle {
      public:
        ...
        // Start the next round: returns the current concurrency group, or
        // state_machine_done once the engine ends the sequence. Call at each
        // join point, including before the first rule.
        int64_t next_group(const TestCase& tc);
        // Returns the index of the next rule to run this round, or
        // state_machine_done once the worker's round budget is used up.
