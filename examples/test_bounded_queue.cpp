        ...
        if (std::holds_alternative<Push>(op)) {
            ...
            if (model_accepted != queue_accepted) {
                std::cerr << "Push disagreement: model=" << model_accepted
                          << " queue=" << queue_accepted
                          << " size=" << queue.size() << " cap=" << cap
                          << std::endl;
                return 1;
            }
        } else {
            ...
            if (model.empty()) {
                ...
                if (queue_result.has_value()) {
                    std::cerr << "Pop from empty queue returned value"
                              << std::endl;
                    return 1;
                }
            } else {
                ...
                if (!queue_result.has_value()) {
                    std::cerr << "Pop from non-empty queue returned nullopt"
                              << std::endl;
                    return 1;
                }
                if (*queue_result != model.front()) {
                    std::cerr << "Pop returned " << *queue_result
                              << " but expected " << model.front() << std::endl;
                    return 1;
                }
